...
2. **重跑全量迁移是安全的**，脚本会复用 `SyncSpaceMapping` 表中已有的映射记录，不会重复创建空间，也不会删除 BISHENG 中已有数据，只新增差异部分。如果确实需要清空重建（比如测试环境重置），加 `--force-recreate` 参数——这会删除同名的旧空间并把所有文件重新下载/上传一遍，正常运维不要用。
